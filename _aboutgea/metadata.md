...
experiment で[必須 protocol セット](https://docs.google.com/spreadsheets/d/1HZs21QDMonbP-vA_5O1R5HiWJjkT8kL3NsVu2GG_kXE/edit#gid=961166619)が異なります。
...
: Labeled extract のアレイへのハイブリダイゼーション、もしくは、ライブラリーのシークエンシングは SDRFにおいて「材料（material）」と「データ（data）」セクションをつなぐ要となるステップです。マイクロアレイ実験ではアレイデザインを示す[Array Design REF](#Array_Design_REF) カラムが [Assay Name](#Assay_Name)
に付加されている必要があります。
...
material に対してユーザが定義する値 (デフォルトでは対応する BioSample の sample_name が使われます)
...
(デフォルトでは対応する BioSample の sample_name が使われます)
...
に対してユーザが定義する値 (デフォルトでは対応する BioSample の sample_name
...
イベントに対してユーザが定義する値 (デフォルトでは対応する BioSample の sample_name
...
(デフォルトでは対応する BioSample の sample_name が使われます) を記入します。以下のカラムが
...
"ESUB000500_Protocol_1" のような一時的な ID で参照されます。アクセッション番号発行後、この ID は
...
Extract を生成したラベル化合物名を記入します。例: Cy3、Cy5、biotin、alexa_546。 以下のカラムが
