...
MetaboBank records are linked to BioProject and BioSample. 
...
Submit experimental raw and processed data files, and identified and predicted metabolites as the Metabolite assignment file (MAF) with pre-defined fields.
...
The BioProject assigns an accession with prefix PRJDB.
...
The BioSample assign accessions with prefix SAMD.

### 5. Create metadata  {#metadata}

[Download a metadata excel]((/metabobank/metadata-e.html)) for your submission type. 
The excel consists of two sheets, IDF and SDRF. 
Submit data of different types as separate studies and group them by a BioProject. For example, BioProject PRJDB100 groups MTBKS1000 (LC-MS) and MTBKS1001 (GC-MS).  
...
Prepare following data files. Submit identified metabolites as the [Metabolite assignment file (MAF)](/metabobank/datafile-e.html). MAF registration is not required but highly recommended.
...
Upload the mass directory under your home. The scp command is as below.
...
MetaboBank assigns a accession number to a study (n is natural number). 
...
Please note that the MetaboBank data will not be exchanged with the EBI [MetaboLights](https://www.ebi.ac.uk/metabolights/).

FAQ: [How to request data release?](/faq/ja/request-release-e.html)

## Reviewer access  {#reviewer-access}

If you want to provide your private data to journal reviewers via password-protected website, 
