...
JPO の特許データには英訳された発明タイトルが記載されている。そのため塩基配列データにおいては，[キーワード検索システム ARSA](//ddbj.nig.ac.jp/arsa/) を使用することにより，関連キーワードから先行技術調査を行うことが可能である。Advanced Search 画面の検索条件で，Division から PAT を選択することにより，特許データに限定した検索することが可能である。
...
日本 DNA データバンク（DDBJ）では，第1回コラムで紹介した [日本特許庁（JPO）](//www.jpo.go.jp/index.html)の特許配列データ（特許データ）以外にも，[韓国特許庁（KIPO）](//www.kipo.go.kr/kpo/user.tdf?a=user.main.MainApp)の特許データの公開も行っている。今回のコラムでは KIPO 特許データの概要を中心に，KIPO 関連機関からのデータ検索方法についても紹介を行いたい。
...
Center（KOBIC）](https://www.kobic.re.kr/kobic/?lang=en)
...
今までの特許コラムでは DDBJ が公開を行っている [日本特許庁(JPO)](//www.jpo.go.jp/index.html) と [韓国特許庁(KIPO)](//www.kipo.go.kr/kpo/user.tdf?a=user.main.MainApp) の塩基配列とアミノ酸配列の特許配列データの紹介を行った。今回のコラムでは，DDBJ 検索ツールを使用した特許データ検索方法，JPO データの公開フラットファイル(FF)に表示される公開番号を取得して <span class="font-red">\*</span>特許電子図書館(IPDL) で特許公開公報を検索する方法について紹介を行いたい。（\*：特許電子図書館へリンクしています）
...
[ARSA](//ddbj.nig.ac.jp/arsa/) では塩基配列データを対象にキーワードによる検索を行うことができるが，アミノ酸データの検索は行うことはできない。表2に，実際に検索する場合が多いと思われる入力条件例を紹介した。
...
今までの特許コラムでは DDBJ が公開を行っている [日本特許庁(JPO)](//www.jpo.go.jp/index.html) と [韓国特許庁(KIPO)](//www.kipo.go.kr/kpo/user.tdf?a=user.main.MainApp) の塩基配列とアミノ酸配列の特許配列データの紹介，DDBJ 検索ツールを使用した特許公開公報(公開公報) の検索方法の紹介を行った。  
...
JPO では出願人が作成した [配列表(ST25)](//www.jpo.go.jp/system/patent/shutugan/bio/gene/enki_amino_baitai.html) を加工し，DDBJ 登録用ファイル(JPO ファイル) が作成している。DDBJ では JPO から送られてくる登録ファイルを変換し，特許データ
FF を構築している。  
...
1.  LOCUS 行- KEYWORDS 行
    ...
    {::nomarkdown}
    <a class="group1 cboxElement" href="/assets/images/ddbj/column_14.jpg" title="アミノ酸配列データの LOCUS 行(例)"><figure><img src="/assets/images/ddbj/column_14.jpg" alt="アミノ酸配列データの LOCUS 行(例)" /><figcaption>図2：アミノ酸配列データの LOCUS 行(例)</figcaption></figure></a>
    {:/}
...
6.  配列情報 
    ...
    <figure><a href="/assets/images/ddbj/column_15.jpg"><img src="/assets/images/ddbj/column_15.jpg" alt="アミノ酸配列データの配列情報の表示"></a><figcaption>図3：アミノ酸配列データの配列情報の表示</figcaption></figure>   
...
## ５．日本特許庁(JPO)データにおける公開フラットファイルの新フォーマットの紹介  <a name="5"></a>
...
[![](/assets/images/ddbj/Fig1rev.jpg){: .float-left}](/assets/images/ddbj/Fig1rev.jpg){: .group1}

[![](/assets/images/ddbj/Fig2rev.jpg){: .float-right}](/assets/images/ddbj/Fig2rev.jpg){: .group1}
...
出願人は配列をJPO へ提出する際に、WIPO ST.25 という [ガイドライン](//www.jpo.go.jp/system/patent/shutugan/bio/gene/enki_amino_guideline/) に従い配列表を作成する。  
...
PS 行は固定文字列N の後に配列表の配列番号が記載されるため、[ARSA](//ddbj.nig.ac.jp/arsa/) で公開番号とN を含めた配列番号をキーワードに指定することにより、該当するエントリーを検索できる様になった。
