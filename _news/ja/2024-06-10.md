---
layout: simple
title: <span class="red">[復旧]</span>(6/14-20) 登録・検索システムの停止
category: news
db:
  - bioproject
  - biosample
  - dra
  - gea
  - jga
  - agd
  - ddbj
  - top
  
tags:
  - メンテナンス

date: 2024-06-10T00:00:01+09:00
retire_time: 2024-06-21T15:00:00+09:00
lang: ja
---

<span class="red">[2024-06-20 15:00]</span>    
全てのサービスが復旧いたしました。    
ご協力ありがとうございました。    

---

<span class="red">[2024-06-18 14:15]</span>    
D-way 関連システムと getentry が復旧しました。    
   

メンテナンスのため getentry, D-way 関連システムとDDBJ 登録受付システムが以下の期間停止いたします。    
皆様には大変ご迷惑をおかけいたしますが、ご理解とご協力をお願いいたします。

---  
2024年6月14日(金) 13:00 - 2024年6月18日(火) 15:00    

- <span class="red">[復旧]</span>D-way 登録システム
	- BioProject
	- BioSample
	- DRA
	- GEA
	- GEA reviewer access
	- DRA/GEA/MSS/MetaboBank SFTP (ftp-private.ddbj.nig.ac.jp) サービス 
- <span class="red">[復旧]</span>getentry

D-way アカウントシステムの停止に伴い以下の機能が利用できなくなります。    
- アカウントの新規登録
- アカウント情報の更新
- パスワードの再設定

---    

---  
2024年6月14日(金) 13:00 - 2024年6月20日(木) 15:00    
以下の DDBJ 登録受付システムが停止します。
- <span class="red">[復旧]</span>NSSS
- <span class="red">[復旧]</span>MSS

---    
