...
ダウンロードした暗号化ファイルは、復号ツールを使って復号します。利用者の解析環境で J-DU999991 ディレクトリ直下に移動し、tools ディレクトリにある復号ツール「J-DU999991.tool.zip」を J-DU999991 ディレクトリ直下に展開します。復号処理スクリプトおよび暗号化済み共通鍵ファイルが展開されます。DU 全体を対象とする復号スクリプト J-DU999991.decrypt.sh が、（tools ディレクトリの下ではなく）J-DU999991 ディレクトリ直下に、個々のデータファイルを対象とする case1.fastq.gz.decrypt.sh のような復号スクリプトが暗号化されたデータファイルが含まれる Data/Analysis ディレクトリに配置されます。
...
再処理完了後、[ダウンロード](#sftp)の手順に従い、復号用ツールのみを再度ダウンロードします。データファイルは再度ダウンロードする必要はありません。  
[データファイルの復号](#decrypt)の手順に従い、データファイルを新しく更新した公開鍵とペアになっている秘密鍵を指定して復号します。

{% include image.html url="books/update-key-for-decryption.png" caption="データセット復号用公開鍵の更新" class="w300" %}
